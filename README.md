...
This assignment will help us understand the differences and similarities between the three major CSPs (AWS, Azure, GCP) and the services they offer for building and deploying remote data and real-time application solutions. As a result, organizations can decide the best CSP based on their strengths and weaknesses for their real time or remote data use case. The scope of this assignment covers the various remote data and real-time application contexts we explored in class over the past semester:
...
- AWS: **AWS AppSync** [4] allows developers "connect their applications and services to data and events with secure, serverless and high-performing GraphQL APIs." The applications through AppSync can access 1+ data sources via a single GraphQL endpoint or combine multiple GraphQL endpoints into a single merged on. For example, AppSync GraphQL allows developers to access data from multiple databases, microservices, or AI models with one GraphQL API request. You can also create GraphQL APIs from DynamoDB or Aurora databases.
  - You can integrate this with any third party GraphQL engine like Hasura, Apollo, or both.
- Azure: **Azure API Management** [5] also supports GraphQL API by passing through to an existing GraphQL endpoint you have or importing a GraphQL schema (via Azure CLI) and creating a synthetic GraphQL API based on that. In general, just like REST API, you can use API Management to manage your GraphQL APIs, either by pass-through (your existing GraphQL service endpoint, like Apollo) or synthetic. (based on custom GraphQL schema)
...
- AWS: **Amazon API Gateway** [7] also supports "collection[s] of WebSocket routes that are integrated with backend HTTP endpoints, Lambda functions, or other AWS services." The predefined routes in API Gateway include "`$connect` and `$disconnect`" which act as the handshake and closing frame respectively. You can use API Gateway to develop, publish, protect, and monitor your WebSocket APIs.
...
- Azure: **Azure API Management and Azure Web PubSub** [9] [10] both allow developers to integrate WebSocket into their ecosystem, either by importing an existing endpoint into a central management plane (API Management) or by using WebSocket protocol as an option to enable realtime messaging via a pub/sub structure, which is ideal for a real-time chat scenario as an example. (Web PubSub)
  - Web PubSub can autoscale to handle thousands of simultaneous connections at once.
- GCP: **Cloud Run** [11] allows developers to run WebSocket applications on Cloud Run as a streaming service. You can do so as simply as building the service with the Google Cloud CLI within the service Git repo. WebSockets benefit greatly from Cloud Run's inherent concurrency, allowing them to maximize their scaling potential for connected clients.
  - Cloud Run is able to autoscale out instances to handle streaming and connection demand. However, developers have to be mindful about synching data between multiple Cloud Run instances, which is made challenging by the stateless and autoscaling nature of Cloud Run.


**d) Data Streaming Services (10%)**

- AWS: **Amazon Kinesis Data Streams** [12]: allows developers "to collect and process large streams of data records in real time" and create Kinesis applications from it. From there, you can use processed records to display on dashboards, generate alerts, dynamically trigger changes in other services, or forward data to other services. Businesses can use Kinesis, for real-time, continuous data ingestion and aggregation, including but not limited to log data, app logs, social media posts, and clickstream data.
- Azure: **Azure Event Hubs** [13] is a data streaming service that can handle millions of events per second with low latency, from any number of sources to any destination. Event Hubs handles both one-way ingestion and storing data streams to gain insights and drive real-time analytics. In essence, it acts as a buffer that ingests data for the destination.
  - **Azure IoT Hubs** [17] is a data streaming service for physical IoT devices, and acts as a central message hub for them. It enables secure and reliable communicate at any scale between IoT devices and applications over multiple web protocols, including but not exclusive to both MQTT and HTTP.
- GCP: **Google Cloud Pub/Sub**  [14] ingests and distributes event data streams to destination sources like the AI-enabled Dataflow. It operates on a pub/sub system with messages/topics. It is designed to fetch and deliver messages instantly across millions of messages all around the world. Google advertises it as an entry point to ingest records at any volume in real-time pipelines.

**e) Stream Analytics (10%)**

- AWS: **Amazon Kinesis Data Stream** [12] can also run real-time analytics on the event data via integrations with AWS Lambda, because it acts as the ingester, bufferer, and processor of streaming data. It is capable of analyzing data streams and emiting results to a data store or dashboard application. Instead of batch processing, Kinesis can perform real-time analytics without storing the raw data partway through.
